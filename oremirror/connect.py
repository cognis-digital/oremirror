"""Native cognis-connect emit for oremirror — forward findings to any platform.

Maps oremirror's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    oremirror ... --format json | oremirror-emit --to stix
    oremirror-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('name', '') + ('-' + rec.get('tag', '') if rec.get('tag') else '')
        out['severity'] = rec.get('severity', 'info')
        out['type'] = rec.get('type', 'unknown')
